...
$db_user = 'u41067';
$db_pass = '34636774';
$db = new PDO('mysql:host=localhost;dbname=u41067', $db_user, $db_pass, array(PDO::ATTR_PERSISTENT => true));
...
if (empty($_SERVER['PHP_AUTH_USER']) || empty($_SERVER['PHP_AUTH_PW']) || $_SERVER['PHP_AUTH_USER'] != 'admin' || $_SERVER["PHP_AUTH_PW"]!="admin") {
...
function show_tables($db){
    $sql = 'SELECT  application6.*, SuperDef.name as power, users6.login FROM application6
...
<?php
    ...
    $sql = 'SELECT SuperDef.name as superpower, COUNT(superpowers) as number_of_users FROM Superpowers6 
                LEFT JOIN SuperDef
                    ON Superpowers6.superpowers=SuperDef.id
...
?>
    <form action="" method="post">
        ...
        <p>
            <button type="submit" value="send">Ок</button>
        </p>
    </form>
<?php
    }
